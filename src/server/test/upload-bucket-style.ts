...
/**
 * Upload test file using virtual-hosted-style (bucket in hostname)
 * URL format: https://mentra-notes.3c764e987404b8a1199ce5fdc3544a94.r2.cloudflarestorage.com/transcripts/...
 */
async function uploadTestFile() {
  loadEnv();

  console.log("\n🚀 R2 Test Upload - Virtual-Hosted-Style (bucket in hostname)\n");
  ...
  // Upload using path-only (no bucket in hostname)
  ...
  console.log(`  R2 Key: /${bucketName}/${key}\n`);

  try {
    // Use path-style with bucket prefix in the path
    const endpoint = `https://${accountId}.r2.cloudflarestorage.com`;
    const fullKey = `${bucketName}/${key}`;
    await uploadToR2(endpoint, fullKey, jsonContent, accessKeyId, secretAccessKey);
