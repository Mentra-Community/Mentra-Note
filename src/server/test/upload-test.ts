...
/**
 * Upload test file to R2 at: mentra-notes/transcripts/aryan.mentra.dev.public@gmail.com/date/transcript.json
 */
async function uploadTestFile() {
  loadEnv();

  console.log("\n🚀 R2 Test Upload - Creating mentra-notes/transcripts/... structure\n");
  ...
  // Upload WITH bucket name in path (token is scoped to mentra-notes/*)
  const date = new Date().toISOString().split("T")[0];
  const userEmail = "aryan.mentra.dev.public@gmail.com";
  const key = `mentra-notes/transcripts/${userEmail}/${date}/transcript.json`;
  ...
  try {
    await uploadToR2(endpoint, key, jsonContent, accessKeyId, secretAccessKey);
