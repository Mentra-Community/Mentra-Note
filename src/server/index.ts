...
import { setupDbRoutes } from "./routes/db.routes";
import { setupSSERoutes } from "./routes/sse.routes";
import mongoose from "mongoose";
import * as path from "path";
import { User } from "../shared/class/User";
import { initializeFile } from "./manager/db.manager";
...
class ExampleMentraOSApp extends AppServer {
  constructor() {
    ...
    setupDbRoutes(this.getExpressApp());

    // Set up SSE routes for transcription streaming
    setupSSERoutes(this.getExpressApp());
  ...
  protected async onSession(
    ...
    // Create user and start transcription
    const user = new User(session);
    user.startTranscription();
    // Initialize user file data
    initializeFile(user.userId);
...
const MONGODB_URI =
  process.env.MONGOURL || "mongodb://localhost:27017/mentra-notes";
